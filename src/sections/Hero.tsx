  ...
  return (
    <section className="relative flex flex-col justify-between h-[100dvh] min-h-[100dvh] w-full py-10 px-6 sm:px-10 select-none overflow-visible bg-paper dark:bg-[#111111]">
      ...
      <div className="relative w-full max-w-5xl mx-auto flex flex-col items-center justify-center">
        {/* Left Spark */}
        <div className="absolute left-[2%] sm:left-[0%] md:left-[-6%] top-[-10%] sm:top-[-8%] md:top-[-6%] z-10 pointer-events-none">
          <img
            src={starImg}
            alt="Star"
            className="w-[80px] h-[80px] sm:w-[110px] sm:h-[110px] md:w-[140px] md:h-[140px] object-contain no-outline"
          />
        </div>

        {/* Right Spark */}
        <div className="absolute right-[2%] sm:right-[0%] md:right-[-6%] bottom-[10%] sm:bottom-[12%] md:bottom-[14%] z-10 pointer-events-none">
          <img
            src={boltImg}
            alt="Lightning Bolt"
            className="w-[90px] h-[90px] sm:w-[125px] sm:h-[125px] md:w-[160px] md:h-[160px] object-contain no-outline"
          />
        </div>

        <h1 className="flex flex-col items-center justify-center font-display text-center select-none max-w-5xl leading-none">
          {/* Line 1 */}
          <span className="text-[7.5vw] sm:text-[8vw] font-black tracking-[-0.03em] text-offblack dark:text-zinc-100 uppercase whitespace-nowrap block min-h-[1.1em]">
            {display.line1 || '​'}
          ...
          {/* Line 2 */}
          <span className="text-[5vw] sm:text-[5.5vw] font-black tracking-[-0.03em] text-offblack dark:text-zinc-100 uppercase whitespace-nowrap block mt-2 min-h-[1.1em]">
            {display.line2 || '​'}
            {!display.cursorOnLine1 && (
              <span className="typewriter-cursor bg-offblack dark:bg-zinc-100" style={{ height: '0.75em', width: '2px' }} />
            )}
      ...
      <div className="flex flex-row items-end justify-between w-full">
        <span className="text-3xl sm:text-4xl font-black text-offblack dark:text-zinc-100 tracking-tight font-sans select-none shrink-0 leading-none mb-4">
          &copy;2026
        </span>
        <div className="w-[140px] h-[175px] shrink-0" />
        <span className="text-[9px] sm:text-[11px] font-bold text-zinc-500 tracking-widest select-none shrink-0 text-right uppercase leading-none pb-1 font-sans">
          /CREATING SINCE 2020
        </span>
      </div>
    </section>
